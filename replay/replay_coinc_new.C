...
void replay_coinc_new(Int_t runnumber=0,Int_t numevents=0,Int_t fstEvt=0,Bool_t QuietRun = kFALSE, Bool_t OnlineReplay =kFALSE, Bool_t bPlots = kFALSE, Bool_t autoreplay = kFALSE){
	...
	if(bPlots){
		...
		gSystem->Exec(Form("mv %stemp_%d.pdf %sleft_detectors_%d.pdf"                  ,SUM_DIR      ,runnumber       ,path_to_plots,runnumber));
		gSystem->Exec(Form("unlink %sleft_detectors_latest.pdf"                        ,SUM_DIR                                               ));
		gSystem->Exec(Form("ln -s %sleft_detectors_%d.pdf %sleft_detectors_latest.pdf" ,path_to_plots,runnumber       ,SUM_DIR                ));
		gSystem->Exec(Form("ln -sf %sleft_detectors_%d.pdf %sleft_detectors_latest.pdf",path_to_plots,runnumber       ,path_to_plots          ));

		gSystem->Exec(Form("%sonline -P -f %s -r %d"                                   ,GUI_DIR      ,CONFIGFILEPHYS_L,runnumber              ));
		gSystem->Exec(Form("mv %stemp_%d.pdf %sleft_physics_%d.pdf"                    ,SUM_DIR      ,runnumber       ,path_to_plots,runnumber));
		gSystem->Exec(Form("unlink %sleft_physics_latest.pdf"                          ,SUM_DIR                                               ));
		gSystem->Exec(Form("ln -s %sleft_physics_%d.pdf %sleft_physics_latest.pdf"     ,path_to_plots,runnumber       ,SUM_DIR                ));
		gSystem->Exec(Form("ln -sf %sleft_physics_%d.pdf %sleft_physics_latest.pdf"    ,path_to_plots,runnumber       ,path_to_plots          ));
		...
		*/
		...
		// RHRS
		//==================================
		const char * CONFIGFILE_R    = Form( REPLAY_DIR_PREFIX,"onlineGUI64/config_coinc_2018/RHRS.cfg"       );
		const char * CONFIGFILEPHYS_R= Form( REPLAY_DIR_PREFIX,"onlineGUI64/config_coinc_2018/RHRS_phy.cfg"   );
		...
		gSystem->Exec(Form("mv %stemp_%d.pdf %sright_detectors_%d.pdf"                   ,SUM_DIR      ,runnumber       ,path_to_plots,runnumber));
		gSystem->Exec(Form("unlink %sright_detectors_latest.pdf"                         ,SUM_DIR                                               ));
		gSystem->Exec(Form("ln -s %sright_detectors_%d.pdf %sright_detectors_latest.pdf" ,path_to_plots,runnumber       ,SUM_DIR                ));
		gSystem->Exec(Form("ln -sf %sright_detectors_%d.pdf %sright_detectors_latest.pdf",path_to_plots,runnumber       ,path_to_plots          ));

		gSystem->Exec(Form("%sonline -P -f %s -r %d"                                     ,GUI_DIR      ,CONFIGFILEPHYS_R,runnumber              ));
		gSystem->Exec(Form("mv %stemp_%d.pdf %sright_physics_%d.pdf"                     ,SUM_DIR      ,runnumber       ,path_to_plots,runnumber));
		gSystem->Exec(Form("unlink %sright_physics_latest.pdf"                           ,SUM_DIR                                               ));
		gSystem->Exec(Form("ln -s %sright_physics_%d.pdf %sright_physics_latest.pdf"     ,path_to_plots,runnumber       ,SUM_DIR                ));    
		gSystem->Exec(Form("ln -sf %sright_physics_%d.pdf %sright_physics_latest.pdf"    ,path_to_plots,runnumber       ,path_to_plots          ));
